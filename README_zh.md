...
- **earmark 注解**：工具栏 earmark 按钮（四向箭头图标），可在任意页面（包括第三方网站）打开圈选 overlay，点击元素添加备注，注解存到本地 broker，Hermes Agent 可通过 MCP 读取。按钮已汉化，支持圈选/文字/框选/冻结四种模式 + 优先级标注
- **Cookie 导入**：工具栏导入按钮，从系统默认浏览器（Chrome/Edge）读取登录态 Cookie 并注入内置浏览器，无需重新登录（需先关闭 Chrome/Edge）
### 技能与记忆
